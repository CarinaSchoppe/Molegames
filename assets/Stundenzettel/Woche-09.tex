\documentclass[a4paper,11pt]{scrartcl}
\usepackage{ngerman}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{array}
\usepackage{longtable}

\voffset-10mm
\hoffset-10mm
\textheight250mm
\textwidth180mm

\begin{document}

\begin{center}
	\textbf{\Large Stundenzettel des Softwaretechnikpraktikums WS 2021}
\end{center}

\bigskip

\begin{tabular}{p{8cm} l}
	\begin{tabular}{l c}
		\Large Gruppe: & \Large 10 \\  %Gruppennummer eintragen
		\\\\
		\Large Woche:  & \Large 9 \\  %Aktuelle Woche eintragen
	\end{tabular}
	&
	\begin{tabular}{l}
		\underline{\small Folgende Tätigkeiten sind zu } \\
		\underline{\small erfassen und zu verfeinern:} \\
		Projektmanagement \\
		Werkzeuge \\
		Dokumentation \\
		Sprint Planning \\
		Backlog \\
		Präsentation \\
		Öffentlichkeitsarbeit \\
		Implementierung \\
		Qualitätsmanagement \\
	\end{tabular}
	\\
\end{tabular}
\bigskip

\newcolumntype{C}[1]{>{\centering\arraybackslash}m{#1}}
\begin{longtable}{| p{13.8cm} | C{2.4cm} |}
	\hline
	\textbf{Tätigkeit} & \textbf{Stundenzahl} \\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Albrecht, Michael} & \textbf{15,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
	Projektmanagement: Wöchentliches Treffen
	&
	%Stundenzahl
	1,5
	\\
	\hline

	%Taetigkeit
	Öffentlichkeitsarbeit: Messe Software präsentieren
	&
	%Stundenzahl
	4
	\\
	\hline

	%Taetigkeit
	Implementierung: Android Tutorials
	&
	%Stundenzahl
	5
	\\
	\hline

	%Taetigkeit
	Projektmanagement: Samstagstreffen mit allen die am Smartphone Teilnehmer arbeiten
	&
	%Stundenzahl
	4
	\\
	\hline

	%Taetigkeit
	Backlog - Warten und Tasks updaten
	&
	%Stundenzahl
	1
	\\
	\hline

	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Bulut, Alp} & \textbf{11,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Projektmanagement: wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3,5
	\\
	\hline

	%Taetigkeit
  Implementierung: File handling
	&
	%Stundenzahl
  6,5
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Cigraksedetmen, Dila} & \textbf{8,5}\\ %Gesamtstunden
	\hline

		%Taetigkeit
  Projektmanagement: wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3,5
	\\
	\hline

	%Taetigkeit
  Implementierung: File handling
	&
	%Stundenzahl
  3,5
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Fromme, Philipp} & \textbf{20}\\ %Gesamtstunden
	\hline

	%Taetigkeit
	Projektmanagement: wöchentliches Treffen
	&
	%Stundenzahl
	1,5
	\\
	\hline

	%Taetigkeit
	Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
	3,5
	\\
	\hline

	%Taetigkeit
  Implementierung: Beobachter Spielfeld
	&
	%Stundenzahl
  15
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Juozapaviciute, Eva} & \textbf{0}\\ %Gesamtstunden
	\hline
	
	%Taetigkeit
  Ausfall: Todesfall in der Familie
	&
	%Stundenzahl
	\\
	\hline


	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Kunkel, Lennart} & \textbf{14}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Projektmanagment: wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Kauf/Kontakt mit Gruppe05
	&
	%Stundenzahl
  2
	\\
	\hline

	%Taetigkeit
  Implementierung: Android Studio Einrichtung und Einarbeitung
	&
	%Stundenzahl
  2
	\\
	\hline

	%Taetigkeit
  Projektmanagment: Messe Planung
	&
	%Stundenzahl
  1
	\\
	\hline

	%Taetigkeit
  Projektmanagement: Samstagstreffen mit allen die am Smartphone Teilnehmer arbeiten
	&
	%Stundenzahl
  4
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Lührmann, Nick} & \textbf{11,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
	Projektmanagement: wöchentliches Treffen
	&
	%Stundenzahl
	1,5
	\\
	\hline

	%Taetigkeit
	Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3
	\\
	\hline

	%Taetigkeit
  Projektmanagement: Samstagstreffen mit allen die am Smartphone Teilnehmer arbeiten
	&
	%Stundenzahl
  4
	\\
	\hline

	%Taetigkeit
  Implementierung: Spielbeitritt Smartphone- Teilnehmer \& Verbindung Server
	&
	%Stundenzahl
  3
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Mani, Issam} & \textbf{19,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Projektmanagment wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Einsatz als Experte bei der Messe
	&
	%Stundenzahl
  3
	\\
	\hline

	%Taetigkeit
  Implementierung: Spielfeld/Beobachter
	&
	%Stundenzahl
  15
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Rademann, Jana} & \textbf{8,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Projektmanagment wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3
	\\
	\hline

	%Taetigkeit
  Implementierung: Ausrichter
	&
	%Stundenzahl
  4
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Schoppe, Carina} & \textbf{29,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Implementierung: Server
	&
	%Stundenzahl
  3
	\\
	\hline

	%Taetigkeit
  Implementierung: KI Player
	&
	%Stundenzahl
  10
	\\
	\hline

	%Taetigkeit
  Implementierung: Client, Netzwerk und Game Logik
	&
	%Stundenzahl
  12
	\\
	\hline

	%Taetigkeit
  Projektmanagement: wöchentliches Treffen
	&
	%Stundenzahl
  1,5
	\\
	\hline
	
	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3
	\\
	\hline
	\hline

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name, Vorname
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

	\textbf{Willberg, Marc} & \textbf{19,5}\\ %Gesamtstunden
	\hline

	%Taetigkeit
  Projektmanagement: wöchentliches Treffen (Aufteilung der Gruppen in Fachbereiche)
	&
	%Stundenzahl
  1,5
	\\
	\hline

	%Taetigkeit
  Implementierung: Gruppenarbeit - Spielfeld erstellen anhand der Informationen des Gamestates mit Philipp und Issam
	&
	%Stundenzahl
  15
	\\
	\hline
	
	%Taetigkeit
  Öffentlichkeitsarbeit: Messe
	&
	%Stundenzahl
  3
	\\
	\hline

\end{longtable}


\end{document}
