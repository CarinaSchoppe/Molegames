 ...
 * File created on 17.01.22, 22:19 by Carina Latest changes made by Carina on 17.01.22, 22:18 All contents of "PacketHandler" are protected by copyright. The copyright law, unless expressly indicated otherwise, is
...
public class PacketHandler {
    ...
    if (packet.getPacketType().equalsIgnoreCase(Packets.LOGIN.getPacketType())) {
      ...
      for (var packets : Packets.values()) {
        if (packets.getPacketType().equalsIgnoreCase(packet.getPacketType())) {
          if (MoleGames.getMoleGames().getServer().isDebug()) {
            System.out.println("The packet: " + packet.getPacketType() + " is not handled by the server!");
            return;
          }
        }
      }
  ...
  public void tournamentStateResponsePacket(@NotNull final ServerThread client) {
    ...
    json.add("tournamentState", JsonParser.parseString(new Gson().toJson(tournament.getTournamentState())));
    object.add("value", json);
  ...
  public void handlePlayerLeavePacket(@NotNull final ServerThread client) {
    if (client.getPlayer().getGame() != null) {
      var game = client.getPlayer().getGame();
      if (game.getCurrentGameState() == GameStates.NOT_STARTED && game.getActivePlayers().contains(client.getPlayer())) {
        playerLeftPacket(client.getPlayer(), game);
      } else if (game.getCurrentGameState() != GameStates.OVER && game.getActivePlayers().contains(client.getPlayer())) {
        MoleGames.getMoleGames().getServer().getPacketHandler().playerKickedPacket(client.getPlayer(), game);
      }
    }
